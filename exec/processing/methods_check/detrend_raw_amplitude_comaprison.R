...
paths <- list.files(path = "data", pattern = "ALL_", full.names = TRUE)

# Load the raw data
load_this <- paths[which(grepl(x = paths, pattern = "ALL_trended_basin") == TRUE)]
raw_df    <- get(load(load_this))


# Load the detrended data
load_this  <- paths[which(grepl(x = paths, pattern = "ALL_detrened") == TRUE)]
