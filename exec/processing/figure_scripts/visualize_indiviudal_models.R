...
raw_path <- list.files("data", "basin_mean.rda", full.names = TRUE)
detrended_path <- list.files("data", "detrended_data.rda", full.names = TRUE)
summary_path   <- list.files("data", "summary_stats.rda", full.names = TRUE)
amplitude_path <- list.files("data", "amplitude.rda", full.names = TRUE)
kurotsis_path <- list.files("data", "Kurtosis", full.names = TRUE)
...
save_rda <- function(data, path = "figs/individual_models/"){
...
save_pdfs <- function(data, path = "figs/pdfs/"){

  model_name <- unique(data$model)

  to_print <- flatten(flatten(data$fig))
