# Purpose: This script creates the figures and kables from the driver output for the cmip rcp85
# experiments.
...
# Directories
BASE <- getwd()
INPUT_DIR  <- file.path(BASE, "output", "cmip", "rcp85")
OUTPUT_DIR <- file.path(BASE, "figs", "cmip", "rcp85")


# Processed Models ------------------------------------------------------------------------------------
models_to_remove_path <- list.files(file.path(BASE, "raw-data", "assumptions"), "models_to_remove.csv", full.names = T)
models_removed_path   <- list.files(file.path(BASE, "inst", "extdata", "cmip", "rcp85"), "removed_data.rda", full.names = T)
raw_data_path <- list.files(file.path(BASE, "inst", "extdata", "cmip", "rcp85"), "basin_mean.rda", full.names = T)
