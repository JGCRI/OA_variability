# Purpose: This script acts as the driver for processing the cmip esmrcp data sets. Note this script
...
BASE <- getwd()
INPUT_DIR  <- file.path(BASE, "inst", "extdata", "cmip", "esmrcp85")
OUTPUT_DIR <- file.path(BASE, "output", "cmip", "esmrcp85")
...
raw_data  <- get(load(data_path)) %>% filter(!grepl("MRI-ESM", model))
...
summary_stats <- tidyr::unnest(get.summary_stats(detrended_data %>% filter(!grepl("MRI-ESM", model))))
