...
#   %>% dplyr::mutate(outlier = ifelse(is_outlier(value), paste0(model), NA)) %>% how it is used
...
internal.time_series <- function(input_df, subtitle = "", legend = TRUE){
  ...
  df %>%
    ggplot2::ggplot(aes(x = date, y = value, group = model, color = model)) +
    ggplot2::geom_line(size = 2) +
    ggplot2::facet_wrap(facets = "basin", ncol = 3, scales = "free") +
    ggplot2::labs(y = paste0(var," ", units), title = "Time Series",
  ...
  if(!legend){
    p + ggplot2::theme(legend.position = "none") -> p
  }
...
vis.KS_scatter_plots <- function(data){
  ...
  data %>%
    ...
    mutate(experiment = if_else(grepl("[H|h]istorical", experiment), "histoircal", "future")) %>%
